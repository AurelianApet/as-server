...
io.on('connection', function(socket){
    ...
    */
    socket.on(SEND_MESSAGE, function (data) {
        ...
        socket.emit(RECEIVE_MESSAGE, { from: data.from, to: data.to, message: data.message, type: data.type });
        socket.broadcast.emit(RECEIVE_MESSAGE, { from: data.from, to: data.to, message: data.message, type: data.type });
    });

    socket.on(RECEIVE_MESSAGE, function (data) {
        log('message', currentUser.name + ' emit message: ' + data.message);
        socket.emit(SEND_MESSAGE, { from: data.from, to: data.to, message: data.message, type: data.type });
        socket.broadcast.emit(SEND_MESSAGE, { from: data.from, to: data.to, message: data.message, type: data.type });
    });
