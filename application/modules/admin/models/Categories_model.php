...
class Categories_model extends CI_Model
{
    ...
    public function getCategories($limit = null, $start = null)
    {
        ...
        $query = $this->db->query('SELECT translations_first.*, (SELECT name FROM categories_translations WHERE for_id = sub_for AND abbr = translations_first.abbr) as sub_is, categories.position FROM categories_translations as translations_first INNER JOIN categories ON categories.id = translations_first.for_id ORDER BY id ASC ' . $limit_sql);
        $arr = array();
    ...
    public function deleteCategorie($id)
    {
        ...
        if (!$this->db->delete('categories_translations')) {
    ...
    {
        $this->db->trans_begin();
        if (!$this->db->insert('categories', array('sub_for' =>0))) {
            log_message('error', print_r($this->db->error(), true));
        }
